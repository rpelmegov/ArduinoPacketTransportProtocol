...
public:
  ...
  // Минимальный размер принимаемого пакета
  byte  receiverPackSizeMin;
  ...
  void SetStartSequence(byte* Seq, int Len)
  {
    ...
    packHead_ss_Len = Len;
    packHead_Len    = Len + 2 + 1;
    packHead        = new byte[packHead_Len];
    
    // Зададим значения
    memcpy(packHead, Seq, Len);
  ...
  {
    // Сбросим счетчики
    packHead[packHead_Len - 3]  = 0;
    packHead[packHead_Len - 2]  = 0;
    packHead[packHead_Len - 1]  = 0;
  ...
  void PushToPack(T* val, int len = 1)
  {
    ...
    // Сумму длин прибавим
    packHead[packHead_Len - 1] += s.len;

    // Посчитаем сумму байт
    uint16_t* sum = (uint16_t*)&packHead[packHead_Len - 3];
  ...
  {
    // Передадим шапку
    SendBytes(packHead, packHead_Len);
  ...
  bool ReceiveDataProcessing(byte b)
  {
      ...
      if (progress < packHead_ss_Len)
      {
        // Заберем первый байт и сравним с заголовком
        if (b == packHead[progress])
        {
      ...
      else
      {
        ...
        if (progress < packHead_ss_Len + 2)
        {
          // Считаем
          controlSum[progress - packHead_ss_Len] = b;
